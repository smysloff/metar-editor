(function () {
  ...
  function disableCheckbox(name, cleanResult = false) {
  ...
  async function getIcaosFromDB(fir) {
  ...
  async function getIcaos(value) {
    ...
    try {
      ...
      return db_icaos.length > 0 ? db_icaos : icaos
    } catch (error) {
  ...
  function update$5(value, element) {
  ...
  function auxiliary$2(value, element) {
    if (element.checked) {
  ...
  function auxiliary$1(value, element) {
    if (element.checked) {
      disableCheckbox.call(this, 'nil', true);
    }
  }

  function update$2(value) {
    const { auto } = this.elements;
    return auto.checked ? value : ''
  }

  function auto(form) {
    form.addHandler('auto', { auxiliary: auxiliary$1, update: update$2 });
  }
  ...
  const visibilities$1 = [-1];

  for (let i = 0;    i < 800;   i += 50)   visibilities$1.push(i);
  for (let i = 800;  i < 5000;  i += 100)  visibilities$1.push(i);
  for (let i = 5000; i < 10000; i += 1000) visibilities$1.push(i);
                                           visibilities$1.push(9999);

  const getBase$1 = (value) => value < 800  ? 50
                           : value < 5000 ? 100
                           : value < 9999 ? 1000
                           :                0;

  function updateVisibility$2() {
    const { result } = this;
  ...
  function visibility(form) {
    ...
    form.addHandler('visibility', {
      ...
      auxiliary(value) {
        ...
        const base = getBase$1(visibilityValue);
        let visibilityRangeValue = visibilities$1.at(-1);

        if (base > 0) {
          const roundedValue = Math.floor(visibilityValue / base) * base;
          visibilityRangeValue = visibilities$1.indexOf(roundedValue);
        }

        visibility_range.value = visibilityRangeValue.toString();
      },

      update: updateVisibility$2,
    ...
    form.addHandler('visibility', {
      ...
      init(elements) {
        ...
        element.setAttribute('max', visibilities$1.length - 1);
        element.value = visibilities$1[0];
      },

      auxiliary(value) {
        value = +value;
        const { visibility } = this.elements;
        visibility.value = value === 0 ? '' : visibilities$1[value];
      },
  ...
  function visibility_minimal(form) {
  ...
  function visibility_vertical(form) {
